...
class HomeFragment : BaseFragment<HomePresenter, HomeModel>(), HomeContract.View, BaseQuickAdapter.RequestLoadMoreListener, SpringView.OnFreshListener, BaseQuickAdapter.OnRecyclerViewItemClickListener {
    ...
    override fun initView(view: View) {
        mRecyclerView = view.findViewById(R.id.rv_list) as? RecyclerView
        springview = view.findViewById(R.id.springview) as? SpringView
        progress = view.findViewById(R.id.progress) as? ProgressActivity
        if (arguments != null) {

            mNewsType = arguments.getString(AppConstant.NEWS_TYPE)
            mNewsId = arguments.getString(AppConstant.NEWS_ID)
        }

        //设置下拉刷新监听
        springview?.setListener(this)
        //设置下拉刷新样式
        springview?.header = RotationHeader(activity)
    ...
    override fun showLoadFailMsg(msg: String) {
        //设置加载错误页显示
        progress?.showError(resources.getDrawable(R.mipmap.ic_launcher), AppConstant.ERROR_TITLE, AppConstant.ERROR_CONTEXT, AppConstant.ERROR_BUTTON) {
            PageIndex = 0
            //请求网络数据
            mPresenter.loadHomeListDataRequest(mNewsType!!, mNewsId!!, PageIndex, false)
        }
    ...
    override fun onItemClick(view: View, position: Int) {
        ...
        NewsDetailActivity.startAction(activity, postId, imgUrl)
    ...
    override fun showLoadCompleteAllData() {
        ...
        try {
            ...
            val view = activity.layoutInflater.inflate(R.layout.progress_loaded, mRecyclerView?.parent as ViewGroup, false)
